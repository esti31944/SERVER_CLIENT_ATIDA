...
const { validUser, UserModel, createToken, validLogin } = require("../models/userModel");
...
router.get("/myEmail",auth, async (req, res) => {
    try{
        ...
        // אותה להציג רק את האימייל מתוך המאפיינים
        res.json(user);
    }
    catch(err){
        console.log(err);
...
router.post("/", async (req, res) => {
    ...
    try {
        ...
        // נרצה להציפן באת הסיסמה בצורה חד כיוונית
        // רמת הצפנה שהיא מעולה לעסק - 10
        ...
        if (err.code == 11000) {
            return res.status(500).json({msg:"Email already in system' try login",code:11000})
        }
        console.log(err)
        res.status(500).json({ msg: "err", err })
    }
